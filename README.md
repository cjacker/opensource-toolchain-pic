...
By the way, it's not neccesary to look into the codes seriously at this time. every PIC model had its' own register set, although most of them are identical, most of the codes can be re-used, but the code you write for a model more likely is not compatible with the other models, it always require some major or minor changes to work.
