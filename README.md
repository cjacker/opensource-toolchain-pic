...
And nowaday, MicroChip acquired ATMEL and already add AVR support to XC8, I have no idea what will happen to AVR eco-system in future. 

I really do not recommend PIC today if you have other choices. Anyway, here is tutorial about opensource toolchain of 8-bit PIC, if you wish to get a feel of PIC MCU. 

**Note** the PIC part of SDCC and gputils are un-maintained now, this tutorial is only for hobby project, do not use it in any product.
...
By the way, it's not neccesary to look into the codes seriously at this time. every PIC model had it's own register set, although most of them are identical, most of the codes can be re-used, but the code you write for a model more likely is not compatible with the other models, it always require some major or minor changes to work.
...
If you do not have 'arduino-cli' installed, use arduino IDE to open the 'pp.ino' and upload it.

Before wire it up to the target PIC development board to program, you need to **make sure**:

* Check the datasheet to identify the 'ICSPDAT', 'ICSPCLK' and 'MCLR' pins.
  - ISCPDAT (data) also called 'PGD' pin
  - ICSPCLK (clock) also called 'PGC' pin
  - MCLR is the RESET pin

* Whether the target MCU is 5v voltage tolerance or not, check the datasheet to find out.
...
Usually these boards use newer PIC MCU models that can not supported directly by opensource toolchain, for example, I have a DM164150 CNANO baord with PIC18F57Q43, and it can not supported by SDCC (need some work to do), thus I can not demo the codes works with opensource toolchain.

Although there is no opensource compiler works with it now, microchip's opensource project '[pymcuprog](https://github.com/microchip-pic-avr-tools/pymcuprog)' can be used to program it.
...
After pymcuprog installed, connect cnano PIC board to PC via USB cable and run:
```
pymcuprog ping
```
...
Program it:
```
pymcuprog write -p /opt/microchip/mplabx/v6.00/packs/Microchip/PIC18F-Q_DFP/1.13.211/scripts/pic18f57q43 -f filename.hex  
```
